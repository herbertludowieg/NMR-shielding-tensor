...
void Tensor::points ( ) {
  ...
  for ( unsigned int i = 0 ; i < phi.size() ; i++ ) {
    ...
    for ( unsigned int j = 0 ; j < theta.size() ; j++ ) {
      point_coords_[i][0].push_back( std::sin(phi[i])*std::cos(theta[j]) );
      point_coords_[i][1].push_back( std::sin(phi[i])*std::sin(theta[j]) );
      point_coords_[i][2].push_back( std::cos(phi[i]) );
    }
  }
  for ( unsigned int i = 0 ; i < point_coords_.size() ; i++ ) {
    g.push_back( std::vector<double> () );
    for ( unsigned int j = 0 ; j < point_coords_[0][0].size() ; j++ ) {
      g[i].push_back( multiplication( point_coords_[i][0][j],
                                               point_coords_[i][1][j],
                                               point_coords_[i][2][j] ) );
    }
  }
  ...
  for ( unsigned int i = 0 ; i < point_coords_.size() ; i++ ) {
    ...
    point_coords_[i].push_back( std::vector<double> () );*/
    for ( unsigned int j = 0 ; j < point_coords_[0][0].size() ; j++ ) {
      point_coords_[i][0][j] = g[i][j] * point_coords_[i][0][j] ;
      point_coords_[i][1][j] = g[i][j] * point_coords_[i][1][j] ;
      point_coords_[i][2][j] = g[i][j] * point_coords_[i][2][j] ;
    }
  }
